...
class Network:
    ...
    def inference(self, inputs, is_training):
        ...
        f_neigh = helper_tf_util.conv2d(f_neigh, 32, [1, 1], 'local_graph', [1, 1], 'VALID', True, is_training)
        f_neigh = tf.reduce_max(f_neigh, axis=2, keepdims=True) + f_layer_fc2
        f_layer_drop = helper_tf_util.dropout(f_neigh, keep_prob=0.5, is_training=is_training, scope='dp1')
    ...
    def dilated_res_block(self, feature, xyz, neigh_idx, d_out, name, is_training):
        ...
        return tf.nn.leaky_relu(tf.concat([f_pc + shortcut, f_gc], axis=-1))

    def global_context_learning(self, xyz, feature, neigh_idx, d_out, name, is_training):
